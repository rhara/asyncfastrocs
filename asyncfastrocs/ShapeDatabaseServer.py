...
try:
    ...
    oechem.OEThrow.Fatal("This script is not available, FastROCS is not supported on this platform.")
            ...
            try:
                ...
                if not results:
                    self.exception = RuntimeError(
                        "Query error, no results to return, check the server log for more information")
            finally:
                self.lock.release()
...
class DatabaseLoaderThread(Thread):
    ...
    def run(self):
        ...
        M = min(M, 10000000)
        m = M//50
        dots = oechem.OEThreadedDots(M, m, "conformers")
        ...
        for score in scores:
            ...
            if not self.moldb.GetMolecule(mcmol, score.GetMolIdx()):
                oechem.OEThrow.Warning(
                    "Can't retrieve molecule %i from the OEMolDatabase, skipping..." % score.GetMolIdx())
                continue
            ...
            if options.GetInitialOrientation() != oefastrocs.OEFastROCSOrientation_Inertial:
                oechem.OEAddSDData(mol, "Opt. Starting Pos.", GetAltStartsString(options.GetInitialOrientation()))
